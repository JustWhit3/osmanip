...
namespace osm {
    ...
    class ProgressBar {
       public:
        ...
        // Default constructor
        /**
         * @brief Construct a new ProgressBar <bar_type>::ProgressBar object.
         * Default constructor which set to null values the main attributes.
         *
        ...
        /**
         * @brief Construct a new ProgressBar <bar_type>::ProgressBar object.
         * Parametric constructor which set to null values the main attributes
         * except max_ and min which will be initialized respectively with max
         * and min.
         *
         * @tparam bar_type The type of the ProgressBar.
        ...
        /**
         * @brief Set the type and style of the ProgressBar. Available:
         * "indicator" ("%", "/100"), "loader" ("#", "■"), "spinner" ("/-\\|").
         *
         * @tparam bar_type The type of the ProgressBar.
        ...
        void setStyle(const std::string& type, const std::string& style) {
            try {
                if (styles_map_.at(type).find(style) !=
                    styles_map_.at(type).end()) {
                    style_ = style;
                    type_ = type;
                } else if (styles_map_.at(type).find(style) ==
                           styles_map_.at(type).end()) {
                    throw agr::except_error_func(
                        "Inserted ProgressBar style", style,
                        "is not supported for this type!");
                } else {
                    throw agr::except_error_func("Inserted ProgressBar type",
                                                 type, "is not supported!");
                }
            } catch (std::out_of_range const& exception) {
                throw agr::except_error_func("Inserted ProgressBar type", type,
                                             "is not supported!");
            }
        }
        ...
        /**
         * @brief Set the style of the complete ProgressBar. Available:
         * "indicator" ("%", "/100"), "loader" ("#", "■"), "spinner" ("/-\\|")
         * and "complete" with both indicator first and loader second.
         *
         * @tparam bar_type The type of the ProgressBar.
         * @param type The type (flavor) of the ProgressBar.
         ...
         */
        void setStyle(const std::string& type, const std::string& style_p,
                      const std::string& style_l) {
            if (styles_map_.at("indicator").find(style_p) !=
                    styles_map_.at("indicator").end() &&
                styles_map_.at("loader").find(style_l) !=
                    styles_map_.at("loader").end() &&
                type == "complete") {
                style_ = style_p + style_l;
                style_p_ = style_p;
                style_l_ = style_l;
                type_ = type;
            } else if (styles_map_.at("indicator").find(style_p) ==
                       styles_map_.at("indicator").end()) {
                throw agr::except_error_func("Inserted indicator style",
                                             style_p,
                                             "is not supported for this type!");
            } else if (styles_map_.at("loader").find(style_l) ==
                       styles_map_.at("loader").end()) {
         ...
         */
        void resetAll() {
            max_ = static_cast<bar_type>(0), min_ = static_cast<bar_type>(0),
            style_ = "", type_ = "", message_ = "", time_count_ = 0,
            ticks_occurred = 0, begin_timer = steady_clock::now(),
         ...
         */
        void resetBrackets() {
            brackets_open_.clear(), brackets_close_.clear();
        }
        ...
        // getStyleComplete
        /**
         * @brief Get the ProgressBar Style variable for "complete" variable
         * type.
         *
        ...
        void update(bar_type iterating_var) {
            ...
            iterating_var_ = 100 * (iterating_var - min_) /
                             (max_ - min_ - agr::one(iterating_var)),
            ...
            if (styles_map_.at("indicator").find(style_) !=
                styles_map_.at("indicator").end()) {
                output_ = feat(crs, "left", 100) + getColor() +
                          std::to_string(
                              static_cast<int32_t>(round(iterating_var_++))) +
                          feat(rst, "color") + getStyle();

                update_output(output_);
            }

            // Update of the loader indicator only:
            else if (styles_map_.at("loader").find(style_) !=
                     styles_map_.at("loader").end()) {
                output_ = feat(crs, "left", 100) + getBrackets_open() +
                          getColor() + getStyle() * width_ +
                          agr::empty_space<std::string> *
                              ((agr::isFloatingPoint(iterating_var) ? 26 : 25) -
                               width_) +
            ...
            else if (style_.find(style_p_) != std::string::npos &&
                     style_.find(style_l_) != std::string::npos &&
                     type_ == "complete") {
                output_ = feat(crs, "left", 100) + getBrackets_open() +
                          getColor() + style_l_ * width_ +
                          agr::empty_space<std::string> *
                              ((agr::isFloatingPoint(iterating_var) ? 26 : 25) -
                               width_) +
                          feat(rst, "color") + getBrackets_close() +
                          getColor() + agr::empty_space<std::string> +
                          std::to_string(
                              static_cast<int32_t>(round(iterating_var_++))) +
                          feat(rst, "color") + style_p_;

                update_output(output_);
            }

            // Update of the progress spinner:
            else if (styles_map_.at("spinner").find(style_) !=
                     styles_map_.at("spinner").end()) {
                output_ =
                    feat(crs, "left", 100) + getColor() +
                    getStyle()[static_cast<uint64_t>(iterating_var_spin_) & 3] +
                    feat(col, "green") +
                    ((agr::roundoff(iterating_var, 1) ==
                      agr::roundoff(max_, 1) - agr::one(iterating_var))
                         ? (static_cast<std::string>(feat(crs, "left", 100) +
                                                     "0"))
                         : "") +
                    feat(rst, "color");
       ...
       private:
        ...
        // remaining_time
        /**
         * @brief Compute the remaining time for the completion of the progress
         * bar.
         *
         * @tparam bar_type The type of the ProgressBar.
        ...
        void remaining_time() {
            ...
            osm::cout << "["
                      << feat(sty, "italics") +
                             "Estimated time left: " + feat(rst, "italics")
                      << feat(col, "green") << minutes_left.count()
                      << feat(rst, "color") << "m " << feat(col, "green")
                      << seconds_left.count() << feat(rst, "color") << "s"
                      << "]" << feat(tcsc, "cln", 0);
        }
        ...
        bar_type max_, max_spin_, min_, iterating_var_, iterating_var_spin_,
            width_;
        std::string style_, style_p_, style_l_, type_, conct_, message_,
            brackets_open_, brackets_close_, output_, color_, time_flag_,
            color_name_;
     ...
     * @return std::ostream& It is the output stream with the given progress bar
    ...
    std::ostream& operator<<(std::ostream& os,
        ...
        os << "Max: " << pb.getMax() << "\n"
           ...
           << "Brackets style: " << pb.getBrackets_open()
           << pb.getBrackets_close() << "\n"
