...
void multi_bars() {
    ...
    auto job3 = [&bars, &prog_float]() {
        for (float i = prog_float.getMin(); i < prog_float.getMax();
             i += 0.1f) {
...
int main() {
    ...
    osm::cout << osm::feat(osm::col, "lt cyan")
              ...
              << osm::feat(osm::col, "white") << osm::redirout.getFilename()
    ...
    /** Important note:
     * ============================================================== When in a
     * loop, you must update file with the newest data by calling flush() once
     * every iteration, otherwise the data will not be properly formatted when
    ...
    if (test_perc_bars) {
        ...
        osm::cout << osm::feat(osm::col, "dk gray")
                  << osm::feat(osm::sty, "italics")
    ...
    osm::cout << '\n'
              << osm::feat(osm::col, "lt cyan") << "Open `"
              << osm::feat(osm::col, "white")
              << osm::feat(osm::sty, "underlined")
              << osm::redirout.getFilename()
              << osm::feat(osm::rst, "underlined")
              << osm::feat(osm::col, "lt cyan") << "` to view output.\n\n"
              << osm::feat(osm::rst, "all") << std::flush;
