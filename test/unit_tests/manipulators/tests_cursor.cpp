...
TEST_CASE("Testing the feat function overload for the crs map.") {
    ...
    for (auto &element_m : osm::crs) {
        ...
        CHECK_EQ(osm::feat(osm::crs, element_m.first, feat_int),
                 (osm::crs.at(element_m.first).first + std::to_string(feat_int) + osm::crs.at(element_m.first).second));
    }
...
TEST_CASE("Testing the go_to function.") {
    static const std::string test_string_goto = "\u001b[" + std::to_string(2) + ";"s + std::to_string(5) + "H"s;
