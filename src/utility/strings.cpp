...
namespace osm {
    ...
    // find_first_alpha
    /**
     * @brief Finds the index of the first alphabetic character in the str,
     * starting from the given position.
     *
     * @param str the string be searched.
     * @param pos the starting position.
     *
     * @return the index of the alpha char if successful, otherwise
     * std::string::npos.
     *
     */
    ...
    /**
     * @brief Formats a string according to the ANSI (Control Sequence
     * Introducer) escape sequences present within the string. Can be used in a
     * loop with appending data by using optional arguments.
     *
     * @param str the string to be formatted.
     * @param last_pos [Optional] the final position (index) of the last string
     * to be formatted.
     ...
     *
     */
    std::string get_formatted_from_ansi(const std::string &str,
                                        int32_t *last_pos, int32_t *last_size) {
        ...
        for (; src_crsr_pos < (int32_t)str.size(); ++src_crsr_pos) {
            ...
            if (ch.front() == '\033') {
                ...
                // Not a complete CSI
                if (csi_str.empty()) {
                    if (src_crsr_pos + 1 < (int32_t)str.size() &&
                        str.at(src_crsr_pos + 1) == '[') {
                ...
                if (ch.front() == '\n' || dst_crsr_pos >= (int32_t)res.size()) {
                    ...
                    // We don't want to overwrite the new line, so we insert it
    ...
    // get_ansi_csi_string
    /**
     * @brief Returns the ANSI CSI (Control Sequence Introducer) in the string
     * beginning at the given position (if present).
     *
     * @example   | 'ESC' | '[' | 'n' |  'code'  |
     *
     ...
     *
     */
    [[maybe_unused]] std::string get_ansi_csi_string(const std::string &str,
                                                     size_t esc_pos) {
        if (str.at(esc_pos) != '\033') {
            return {};
        }

        return str.substr(esc_pos,
                          find_first_alpha(str, esc_pos + 2) - esc_pos + 1);
    }

    // get_ansi_csi_number
    /**
     * @brief Returns the number (n) of the ANSI CSI (Control Sequence
     * Introducer) string.
     *
     * @example   | 'ESC' | '[' | 'n' |  'code'  |
     *
    ...
    [[maybe_unused]] int32_t get_ansi_csi_number(const std::string &csi) {
        ...
        if (n_pos < csi.size()) {
            ...
            if (size_t alpha_pos = find_first_alpha(csi, n_pos);
                alpha_pos != std::string::npos) {
                try {
                    number = std::stoi(csi.substr(n_pos, alpha_pos - n_pos),
                                       nullptr);
    ...
    // get_ansi_csi_code
    /**
     * @brief Returns the command code of the ANSI CSI (Control Sequence
     * Introducer) string.
     *
     * @example   | 'ESC' | '[' | 'n' |  'code'  |
     *
     ...
     * @throws std::invalid_argument if the string is not an ANSI CSI.
     *
     * @return the code of the ANSI CSI if found. Otherwise, returns a null
     * terminator.
     *
     */
    ...
    // handle_csi
    /**
     * @brief Given a string of an ANSI CSI (Control Sequence Introducer), this
     * function will execute the CSI and correctly modify the destination string
     ...
     * @param dst_crsr_pos the current position (index) of the destination
     * string
     *
     */
    [[maybe_unused]] void handle_csi(const std::string &csi_str,
                                     std::string &dst_str,
        ...
        int32_t curr_pos = *dst_crsr_pos < (int32_t)dst_str.size()
                               ? *dst_crsr_pos
                               : (int32_t)dst_str.size();
        ...
        switch (code) {
                /**
                 * @brief Moves the cursor n (default 1) cells in the given
                 * direction. If the cursor is already at the edge of the
                 ...
                 */
            case 'A': {
                for (size_t line_count = 0; (int32_t)line_count < number;
                     ++line_count) {
                ...
                if (curr_pos < 0) {
                    *dst_crsr_pos = 0;
                    ...
                    for (int32_t prev_pos = curr_pos - 1; prev_pos >= 0;
                         --prev_pos, ++line_len) {
            ...
            case 'B': {
                for (size_t line_count = 0; (int32_t)line_count < number;
                     ++line_count) {
                    for (; curr_pos >= 0 && curr_pos < (int32_t)dst_str.size();
                         ++curr_pos, ++line_len) {
                        if (char ch = dst_str.at(curr_pos); ch == '\n') {
                            if (first_line) {
                                // Get the original position of the cursor on
                                // the line
                ...
                if (curr_pos >= (int32_t)dst_str.size()) {
                    *dst_crsr_pos = (int32_t)dst_str.size();
                    ...
                    for (int32_t next_pos = curr_pos;
                         next_pos < (int32_t)dst_str.size();
                         ++next_pos, ++line_len) {
            ...
            case 'D': {
                // "ESC[100D" is not always enough to return to the edge of the
                // screen, so guarantee it by increasing the number
                number = (number == 100) ? (int32_t)dst_str.size() : number;

                for (int32_t prev_pos = curr_pos - 1;
                     prev_pos >= 0 && line_len < number;
                     --prev_pos, ++line_len) {
                    if (dst_str.at(prev_pos) == '\n') {
                        break;
                    }
                ...
                /**
                 * @brief If n is 0 (or missing), clear from cursor to end of
                 * screen. If n is 1, clear from cursor to beginning of the
                 * screen. If n is 2 or 3, clear entire screen.
                 *
                 * @example ESC[nJ
                 *
                 */
            case 'J': {
                ...
                /**
                 * @brief If n is 0 (or missing), clear from cursor to the end
                 * of the line. If n is 1, clear from cursor to beginning of the
                 * line. If n is 2, clear entire line. Cursor position does not
                 * change.
                 *
                 * @example ESC[nK
                 *
            ...
            case 'K': {
                if (number == 0) {
                    for (; curr_pos < (int32_t)dst_str.size();
                         ++curr_pos, ++line_len) {
                        if (char ch = dst_str.at(curr_pos); ch == '\n') {
                            break;
                        }
