...
namespace osm {
    ...
    /**
     * @brief Formats a string according to the ANSI (Control Sequence Introducer) escape sequences present within the
     * string. Can be used in a loop with appending data by using optional arguments.
     *
    ...
    // get_ansi_csi_string
    /**
     * @brief Returns the ANSI CSI (Control Sequence Introducer) in the string beginning at the given position (if
     * present).
     *
     * @example   | 'ESC' | '[' | 'n' |  'code'  |
     *
    ...
    // handle_csi
    /**
     * @brief Given a string of an ANSI CSI (Control Sequence Introducer), this function will execute the CSI and
     * correctly modify the destination string and update the current position.
     *
    ...
    [[maybe_unused]] void handle_csi(const std::string &csi_str, std::string &dst_str, int32_t *dst_crsr_pos) {
        ...
        switch (code) {
            ...
            case 'D': {
                // "ESC[100D" is not always enough to return to the edge of the screen, so guarantee it by increasing
                // the number
                ...
                /**
                 * @brief If n is 0 (or missing), clear from cursor to end of screen. If n is 1, clear from cursor to
                 * beginning of the screen. If n is 2 or 3, clear entire screen.
                 *
                 * @example ESC[nJ
                 *
                 */
