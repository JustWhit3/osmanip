...
namespace osm {
     ...
     *
     */
    Decorator::~Decorator() { osm::cout << feat(rst, "all"); }
    ...
    void Decorator::removeStyle(const std::string &style, std::ostream &os) {
        ...
        if (getStyleList()[&os][0] == ' ')
            styles.at(&os).erase(0, 1);
        else if (getStyleList()[&os].back() == ' ')
            styles.at(&os).pop_back();
    }
